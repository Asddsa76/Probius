...
async def lfg(channel, text, client):
    ...
    roles = [
        i for i in channel.guild.roles if i.name.lower().replace(" ", "") in inputRoles
    ]
    people = [
        i
        for i in channel.guild.members
        if len(roles) == sum(1 for j in roles if j in i.roles)
    ]
    lfgRole = client.get_guild(535256944106012694).get_role(693038480783048774)
