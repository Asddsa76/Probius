...
async def deleteMessages(author, ping, client):
    ...
    for channel in guild.text_channels:
        try:
            ...
            pass
    await guild.get_channel(576018992624435220).send(
        "Deleted " + str(deletedCount) + " messages from " + ping
    )


async def removeEmbeds(
    message,
):  # Some embeds are instant, others are edited in by discord. Call in both on_message and on_message_edit
    if message.embeds:
        for i in [
            "forums.blizzard.com",
            "psionic-storm.com",
            "heroespatchnotes.com",
        ]:  # Forum embeds are huge image, psionic-storm builds/talent embeds link to wrong build number or blank calculator
...
async def waitList(message, text, client):
    if len(text) == 1:
        await message.channel.send(
            "Wait list: " + " ,".join([i.name for i in client.waitList])
        )
    elif text[1] in ["join", "next"]:
        client.waitList.append(message.author)
        await message.channel.send(
            message.author.name + " has been added to the wait list."
        )
    elif text[1] in ["ping", "here"]:
        await message.channel.send(
            "Wait list: " + ", ".join([i.mention for i in client.waitList])
        )
    elif text[1] == "clear":
        client.waitList = []
