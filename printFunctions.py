...
def deepAndShallowSearchFoundBool(
    ability, string, deep
):  # Python3.5 doesn't allow async functions inside list comprehension :(
    if not deep:
...
async def printSearch(
    abilities, talents, name, hero, deep=False
):  # Prints abilities and talents with the name of the identifier
    ...
    for ability in abilities:
        if (
            sum(
                [1 for i in namelist if deepAndShallowSearchFoundBool(ability, i, deep)]
            )
            == len(namelist)
            and exclude not in ability.lower()
        ):
            ...
            if (
                sum(
                    [
                        1
                        for i in namelist
                        if deepAndShallowSearchFoundBool(talent, i, deep)
                    ]
                )
                == len(namelist)
                and exclude not in talent.lower()
            ):
                output += await addUnderscoresAndNewline(namelist, talent)
    return output


async def printLarge(
    channel, inputstring, separator="\n"
...
async def printAll(
    client, message, keyword, deep=False
):  # When someone calls [all/keyword]
    if len(keyword) < 4 and message.author.id != 183240974347141120:
        await message.channel.send(
            "Please use a keyword with at least 4 letters minimum"
        )
        return
    toPrint = ""
    ...
    if toPrint == "":
        return
    botChannels = {
        "Wind Striders": 571531013558239238,
        "The Hydeout": 638160998305497089,
        "De Schuifpui Schavuiten": 687351660502057021,
    }
    if (
        toPrint.count("\n") > 5 and message.channel.guild.name in botChannels
    ):  # If the results is more lines than this, it gets dumped in specified bot channel
        channel = message.channel.guild.get_channel(
            botChannels[message.channel.guild.name]
        )
        introText = (
            message.author.mention + ", Here's all heroes' " + '"' + keyword + '":\n'
        )
