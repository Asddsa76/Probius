...
def trimForHeroesTalents(hero):
    ...
    for i in remove:
        hero = hero.replace(i, "")
    hero = (
        hero.replace("butcher", "thebutcher")
        .replace("ú", "u")
        .replace("cho", "chogall")
    )
    return hero
...
async def additionalInfo(hero, name, description):
    addDict = {  # Adds text to the end of descriptions
        ...
        "chen": {
            "Storm, Earth, Fire": "Using Storm, Earth, Fire removes most negative effects from Chen."
        },
        "guldan": {"Life Tap": "Costs 222 (+4% per level) Health."},
        "johanna": {"Heaven's Fury": "4 bolts per second per enemy, up to 2 enemies."},
        "tyrande": {
            "Huntress' Fury": "Splashes do not trigger any of Tyrande's Basic Attack related effects."
        },
        "tracer": {
            "Ricochet": "Ricochet shots do not interact with Bullet Time or Focus Fire."
        },
        "imperius": {
            "Impaling Light": "The damage bonus is per brand and stacks to 375%"
        },
        "malfurion": {
        ...
        "tassadar": {
            "Psychic Shock": "Psionic rift deals 2 additional ticks of damage."
        },
    }
    if hero in addDict:
...
async def fixTooltips(hero, name, description):
    fixDict = {  # Replaces text using strikethrough
        ...
        "tyrande": {
            "Light of Elune": [
                "damages an enemy",
                "Auto Attacks an enemy, or deals spell damage to an enemy hero",
            ]
        },
...
async def downloadHero(hero, client, patch):
    async with aiohttp.ClientSession() as session:
        if patch == "":
            page = await fetch(
                session,
                "https://raw.githubusercontent.com/heroespatchnotes/heroes-talents/master/hero/"
                + hero
                + ".json",
            )
        else:
            page = await fetch(
                session,
                "https://raw.githubusercontent.com/MGatner/heroes-talents/"
                + patch
                + "/hero/"
                + hero
                + ".json",
        ...
        if hero == "samuro":
            abilities.append(
                "**[D] Image Transmission:** *14 seconds;* Activate to switch places with a target Mirror Image, removing most negative effects from Samuro and the Mirror Image.\n**Advancing Strikes:** Basic Attacks against enemy Heroes increase Samuro's Movement Speed by 25% for 2 seconds."
            )
        ...
        for key in keys:
            ...
            for talent in tier:
                output = (
                    "**["
                    + str(int(key) - 2 * int(hero == "chromie" and key != "1"))
                    + "] "
                )
                output += talent["name"] + ":** "
...
async def loopFunction(client, heroes, patch):
    for future in asyncio.as_completed(
        map(downloadHero, heroes, repeat(client), repeat(patch))
    ):
        await future
...
async def downloadAll(client, argv):
    ...
    )  # running instead of event when calling from a coroutine. But running is for python3.7+
...
async def heroStats(hero, channel, allowRecursion=True):
    if hero == "The_Lost_Vikings":
        ...
        async with aiohttp.ClientSession() as session:
            page = await fetch(
                session,
                "https://heroesofthestorm.gamepedia.com/index.php?title=Data:" + hero,
            )
            ...
            for i in usefulStats:
                ...
                output.append(
                    ...
                    .capitalize()
                    + "**: "
                    + str(page[: page.index("</td>")]).replace("\n", "")
                )
