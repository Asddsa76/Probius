...
async def mainProbius(client, message, texts):
    ...
    for i in draftAliases:  # Don't want to log draft commands because they really spam.
        ...
        guildname = (
            "Nexus school" if guildname == "Nexus Schoolhouse" else guildname
        )  # Nexus Schoolhouse is too long
        ...
        loggingMessage = (
            ...
            + " " * (18 - len(str(message.author.name)))
            + " "
            + message.content
        )
    ...
    for text in texts:
        ...
        if hero in mapImageAliases:
            await message.channel.send(
                file=discord.File("Maps/" + (await mapAliases(text[1])) + ".jpg")
            )
            continue
        if hero == "core":
        ...
        if hero in lfgAlises:
            ...
            continue
        if hero == "d" and message.channel.id not in [
            643996299678449684,
            643975810256076820,
            643975833970540555,
            672921422544502952,
        ]:  # [D] outside of drafting channel, trait
            continue
        ...
        if hero in updatePokedexAliases:
            ...
            await message.channel.send(
                "Pokedex is down for maintennce. Try again *soon™️*."
            )
            continue
        ...
        if hero == "unsorted" and message.channel.guild.name == "Wind Striders":
            if 557521663894224912 in [
                ...
                rulesChannel = channel.guild.get_channel(
                    634012658625937408
                )  # server-rules
                await channel.send(
                    "Note to all "
                    ...
                    + " and ping **Olympian(mod)** with the **bolded** info at top **`Region`, `Rank`, and `Preferred Colour`** to get sorted before Blackstorm purges you <:peepoLove:606862963478888449>"
                )
        ...
        if output == "":
            if tier.isdigit():  # Talent tier
                ...
                output = printTier(
                    talents,
                    int(tier / 3) + int(hero == "Chromie" and tier not in [1, 18]),
                ...
                if hero == "Deathwing":
                    ...
                    output = abilities[
                        -1
                    ]  # Last ability. It's heroic if the hero has normal mount, but that's an user error
            elif tier == "extra":
                if hero in [
                    "Zeratul",
                    "Gazlowe",
                    ...
                    "Gall",
                    "Stitches",
                ]:  # Some heroes have the entry for 1 button between D and Q, these have them last
                    output = abilities[-1]
                else:
                    output = abilities[1]
                ...
                if (
                    hero == "Tracer"
                ):  # She starts with her heroic already unlocked, and only has 1 heroic
                    output = abilities[3]
                else:
                    output = printTier(
                        talents, 3 - 2 * int(hero == "Varian")
                ...
                tier in wikipageAliases
            ):  # Linking user to wiki instead of printing everything
                await message.channel.send(
                    "<https://heroesofthestorm.gamepedia.com/Data:" + hero + "#Skills>"
                )
                continue
            else:
        ...
        if (
            len(output) == 2
        ):  # If len is 2, then it's an array with output split in half
...
class MyClient(discord.Client):
    ...
    async def on_ready(self):
        ...
        self.forwardedPosts = []
        self.seenTitles = await fillPreviousPostTitles(
            self
        )  # Fills seenTitles with all current titles
    ...
    async def on_message_delete(self, message):
        if message.content and message.channel.guild.id == 535256944106012694:  # WS
            await printLarge(
                client.get_channel(694876665020678175),
                "``"
    ...
    async def on_message(self, message):
        ...
        if "[" in message.content and "]" in message.content:
            ...
            await mainProbius(
                self, message, [message.content.split("[")[1].lower().split("/")]
            )
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if message.id == 693380327413907487:
            if str(payload.emoji) in wsReactionRoles:
                await client.get_guild(535256944106012694).get_member(
                    payload.user_id
                ).add_roles(
                    client.get_guild(535256944106012694).get_role(
                        wsReactionRoles[str(payload.emoji)]
                    )
                )
        elif (
            message.author.id == 603924594956435491 and str(payload.emoji) == "👎"
            ...
            message.author.id == 603924594956435491
            and "React to ping" in message.content
        ):  # Message from Probius, pings Pokedex:
        ...
        if payload.channel_id == 643972679132774410:  # ZH Mockdrafting-role
            await client.get_guild(623202246062243861).get_member(
                payload.user_id
            ).add_roles(
                client.get_guild(623202246062243861).get_role(643975988023394324)
            )
    ...
    async def on_member_join(self, member):
        ...
        if guild.name == "Wind Striders":
            ...
            await channel.send(
                ...
                + "! Please read "
                + rulesChannel.mention
                + " and ping **Olympian(mod)** with the **bolded** info at top **`Region`, `Rank`, and `Preferred Colour`** separated with commas, to get sorted and unlock the rest of the channels <:OrphAYAYA:657172520092565514>"
            )
            try:
                await self.lastWelcomeImage.delete()
            except:
                pass
            self.lastWelcomeImage = await channel.send(
    ...
    async def on_member_remove(self, member):
        ...
        if guild.name == "Wind Striders":
            ...
            await channel.send(
                member.name + " left the server <:samudab:578998204142452747>"
            )
    ...
    async def bgTaskUNSORTED(self):
        ...
        while not self.is_closed():
            ...
            await channel.send(
                ...
                + ": Please read "
                + rulesChannel.mention
                + " and ping **Olympian(mod)** with the **bolded** info at top **`Region`, `Rank`, and `Preferred Colour`** separated with commas, to get sorted and unlock the rest of the channels Blackstorm purges you <:peepoLove:606862963478888449>"
            )
