...
async def mapString(battleground):
    output = (
        battleground.replace("-", " ").title().replace("Of", "of").replace("The", "the")
    )
    return output
...
async def coreAbilities(channel, battleground):
    ...
    page = "".join(
        i
        for i in [
            urlopen("https://nexuscompendium.com/battlegrounds/" + battleground)
            .read()
            .decode("utf-8")
        ]
    ).split("Core Ability - ")[1]
