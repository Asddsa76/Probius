...
def simplifyName(
    hero,
):  # Shorten all names with 10+ letters, and turn underscores into spaces
    hero = hero.replace("_", " ")
    return hero
...
async def printDraft(
    client, channel, draftList
):  # Print state, and the next action to be done
    ...
    output += (
        ", ".join(bansA)
        + " " * (whitespaceAmount - len(", ".join(bansA)))
        + ", ".join(bansB)
        + "\n"
        + "-" * (whitespaceAmount + 15)
        + "\n"
    ...
    if len(draftList) == 17:
        ...
        if channel.guild.id == 623202246062243861:  # Hydeout
            await channel.guild.get_channel(643976359303184404).send(
                output + "```"
            )  # discussion
    else:
