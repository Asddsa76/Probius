...
async def proxyReact(client, text, message):
    await (await message.channel.fetch_message(int(text[1]))).add_reaction(
        client.proxyEmojis[text[0]]
    )
...
async def getProxyEmojis(guild):  # From PythonTest guild
    ...
    for emoji in guild.emojis:
        proxyEmojis[emoji.name] = (
            "<" + "a" * emoji.animated + ":" + emoji.name + ":" + str(emoji.id) + ">"
        )
    return proxyEmojis
...
async def carbotSpray(hero, channel):
    ...
    if hero == "Deflect":
        ...
        imageFormat = ".png"
    emojiPage = (
        "https://heroesofthestorm.gamepedia.com/File:" + hero + "_Spray" + imageFormat
    )
    ...
    if text[0] == "proxy":
        ...
        await channel.send(
            "".join(
                [
                    i[1]
                    for i in client.proxyEmojis.items()
                    if i[1].split(":")[1] not in hiddenEmojis
                ]
            )
        )
        return
...
def downloadEmojis():
    ...
    page = [
        i.strip()
        .decode("utf-8")
        .split('" class="image"><img alt="Emoji ')[1]
        .split('" width=')[0]
        for i in urlopen(EmojiListPage)
        if '<td align="center"><a href="/File:Emoji' in i.strip().decode("utf-8")
    ]
    for i in page:
