...
async def patchNotes(channel, text):
    if len(text) != 2:
        await channel.send(
            "Use with [patchnotes/hero,X], where X is number of patches ago, or a search term."
        )
        return
    text = text[1]
    splitText = text.split(",")
    hero = (
        aliases(splitText.pop(0))
        .replace("ú", "u")
        .lower()
        .translate({ord(i): None for i in "' _-."})
        .replace("thebutcher", "butcher")
    )
    async with aiohttp.ClientSession() as session:
        page = await fetch(
            session, "https://heroespatchnotes.com/hero/" + hero + ".html"
        )
        ...
        if splitText:
            ...
            if text.isdigit():
                if int(text) < len(patches):
                    ...
                    await channel.send(
                        hero.capitalize() + " doesn't have " + text + " patches yet."
                    )
                    return
            else:
        ...
        patch = (
            ...
            .replace("\n**Abilities:**", "")
            .replace("**Stats:**", "")
            .replace(" Health Regen ", ", Health Regen ")
        )
        output += patch
