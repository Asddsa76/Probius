...
async def rotation(channel):
    async with channel.typing():
        page = [
            ...
            for i in urlopen("https://nexuscompendium.com/currently.php")
        ]
        rotationHeroes = []
        salesHeroes = []
        gemPrices = []
        ...
        for line in page:
            if "Special Free-to-Play Period " in line:
                ...
                output = (
                    "**Free rotation "
                    + line
                    + ":** from <https://nexuscompendium.com/>\n"
                )
                ...
                if "Heroic Deals and Limited-Time Items " not in line:
                    output = (
                        "**Free rotation w"
                        + line[lineIndex + 1 : lineIndex + 18]
                        + ":** from <https://nexuscompendium.com/>\n"
                    )
                else:
                    saleWeek = (
                        "**Heroic Deals and Limited-Time Items w"
                        + line[lineIndex + 1 : lineIndex + 18]
                        + ":**\n"
                    )
            elif '<td valign="top" ' in line:
                if len(rotationHeroes) < 14 and "All Heroes" not in rotationHeroes:
                    rotationHeroes.append(
                        line[line.index('title="') + 7 : line.index('" alt')]
                    )
                else:
                    salesHeroes.append(
                        line[line.index('title="') + 7 : line.index('" alt')]
                ...
                if hero not in limitedHeroSkins:
                    ...
                    limitedHeroSkinsVariations[limitedHeroSkins.index(hero)].append(
                        variant
                    )
            elif '#mounts">' in line:
                line = line.split('#mounts">')[1]
                try:
                    limitedMounts.append(
                        line[: line.index("<ul><li>Added")].replace("</a>", "")
                    )
                ...
                goldMounts.append(
                    line.split('">')[1].split("<ul><li>")[0].replace("</a> -", "")
                )
            ...
            output += (
                ...
                + ", ".join(
                    [
                        ...
                        + " "
                        + gemPrices[i]
                        + "<:nexusGem:697311645248127006>"
                        for i in range(len(salesHeroes))
                    ]
                )
                + "\n"
            )
            ...
            output += (
                ...
                + "\n ".join(
                    [
                        limitedHeroSkins[i]
                        + "("
                        + ", ".join(limitedHeroSkinsVariations[i])
                        + ")"
                        for i in range(len(limitedHeroSkins))
                    ]
                )
                + "\n"
            )
