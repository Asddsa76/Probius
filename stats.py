...
class Hero(object):
    def __init__(self, heroInfo, gamemode):
        ...
        self.ci = str(
            round(
                1.96
                * (float(self.wr) * (100 - float(self.wr)) / int(self.games)) ** 0.5,
                2,
            )
        )
...
async def printHeroes(heroes, gamemode, totalGames, channel):
    output = [
        "Score results QM patch 2.48, "
        + str(totalGames)
        + " games total from <https://heroesprofile.com/>"
    ]
    if gamemode == "qm":
        output.append(
            "Hero      Win%   95%CI  Pop% |Hero      Win%   95%CI  Pop% |Hero      Win%   95%CI  Pop% "
        )
        output.append(
            "-----------------------------+-----------------------------+-----------------------------"
        )
        ...
        output.append(
            "Hero          Winrate    95%CI   Ban%     Pop%   Games | Hero          Winrate    95%CI   Ban%     Pop%   Games"
        )
        output.append(
            "                                                       |                                                       "
        )
    ...
    for i in range(m):
        try:
            output.append(
                heroes[i].heroString()
                + " |"
                + heroes[i + m].heroString()
                + " |"
                + heroes[i + 2 * m].heroString()
            )
        except:
            pass
    if len(heroes) % 3 == 1:
        ...
        output.append(
            heroes[m - 1].heroString() + " |" + heroes[2 * m - 1].heroString()
        )
...
async def getData(patch, gamemode):
    ...
    for i in [
        ...
        for i in urlopen(
            "https://www.heroesprofile.com/Global/Hero/?timeframe_type="
            + timeFrameType
            + "&timeframe="
            + patch
            + "&type=win_rate&role=&hero=&game_type="
            + gamemode
        )
    ]:
        if "<a alt=" in i:
    ...
    for hero in heroes:
        hero.pop = str(
            round(100 * int(hero.games) / totalGames, 2)
        )  # Sum from heroesProfile is 984, 10 times larger?
