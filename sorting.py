...
async def sort(roles, member, olympian, client):
            ...
            for i in sorted(guild.roles):
                if (i < bestBot and i > IM) or (i < lopez and i > coreMember):
                    if await trim(i.name) == await trim(role):
                        rolesToAdd.append(i)
                    elif await trim(i.name) == await trim(
                        "".join([i for i in role if not i.isdigit()])
                    ):  # Rank numbers
                        rolesToAdd.append(i)
    ...
    if len(rolesToAdd) != 3:
        await channel.send(
            "At least one role was wrong or inaccessible. Valid roles: "
            + ", ".join([i.name for i in rolesToAdd])
        )
        return
...
async def sortFromMessage(text, message, client):
    ...
    unsortedMember += ">"
    guild = client.get_guild(535256944106012694)  # Wind Striders
    unsortedMember = guild.get_member(
        int(unsortedMember.replace(" ", "")[2:-1].replace("!", ""))
    )
