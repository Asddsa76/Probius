...
async def pokedex(client, channel, hero):
    ...
    if hero == "Samuro":
        names = [
            i.name
            for i in client.get_guild(535256944106012694)
            .get_role(557550150109888513)
            .members
        ]
        message = await channel.send(
            hero
            + " discussion! Our mains are the **Illusion masters: "
            + ", ".join(names)
            + ".** React to ping them."
        )
    elif mains:
        names = [
            client.get_user(int(i)).name
            for i in mains.replace(">", "")
            .replace(" ", "")
            .replace("!", "")
            .split("<@")[1:]
        ]
        ...
        message = await channel.send(
            hero
            + " discussion! We don't have any mains for this hero, ping <@329447886465138689> if you know any. React to ping Balance team."
        )
...
async def pingPokedex(client, message, member):
    ...
    if "Balance team" in message.content:
        await message.channel.send(
            member.mention
            + " wants to start a"
            + "n" * (message.content[0] in vowels)
            + " "
            + message.content.split("We ")[0]
            + "<@&577935915448795137>"
        )
    elif "Samuro" in message.content:
        await message.channel.send(
            member.mention
            + " wants to start a Samuro discussion! <@&557550150109888513>"
        )
    else:
        pokedex = await fillPokedex(client)
...
async def updatePokedex(client, text, message):
    ...
    if heroPing[0] in ["remove", "delete", "all"]:
        ...
        await message.channel.send(
            heroPing[1] + " has been removed from all pokedex entries."
        )
        return
...
async def removePokedex(client, MemberID):  # Removes an user from all pokedex heroes
    ...
    async for message in pokedexChannel.history(limit=50):
        ...
        await message.edit(
            content=message.content.replace(" <@" + MemberID + ">", "").replace(
                " <@!" + MemberID + ">", ""
            )
        )
